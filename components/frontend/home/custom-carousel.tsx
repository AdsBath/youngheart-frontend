        ...
        >
          <CarouselContent className="w-full h-full">
            {bannerData?.map((bannerItem: any) => (
              <CarouselItem className="pb-0 px-0" key={bannerItem?.id}>
                <div className="w-full h-full relative">
                  ...
                  <Image
                    ...
                    fill
                    onLoadingComplete={() => handleImageLoad(bannerItem.id)}
                    className={cn(
                      "transition duration-200 w-full h-full md:object-cover",
                      loaded[bannerItem.id] ? "blur-none" : "blur-md"
                    )}
                    blurDataURL={bannerItem?.imageUrl}
                    placeholder="blur"
                    alt={bannerItem?.altText || "Carousel image"}
                    priority={true}
                  />
                  {bannerItem?.linkUrl && (
                    <Link
