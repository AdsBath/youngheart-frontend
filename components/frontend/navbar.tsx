...
export function MobileDropdown({ category, isOpen, onToggle, onLinkClick }: MobileDropdownProps) {
    return (
        <div className="border-b border-gray-100 bg-white">
            ...
            >
                <div className="flex items-center space-x-3">
                    <div className={`w-1 h-6 rounded-full transition-all duration-200 ${isOpen ? 'bg-orange-500' : 'bg-gray-300'}`}></div>
                    <span className="text-base">{category.title}</span>
                </div>
                <motion.div
                    animate={{ rotate: isOpen ? 180 : 0 }}
                    transition={{ duration: 0.3, ease: "easeInOut" }}
                    className={`transition-colors duration-200 ${isOpen ? 'text-orange-500' : 'text-gray-400'}`}
                >
                    ...
                    >
                        <div className="px-6 py-4">
                            ...
                            >
                                ...
                                <p className="text-xs text-gray-500 mt-1">Explore our {category?.title.toLowerCase()} products</p>
                            </motion.div>
...
const Navbar: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (categoriesData) {
            const freshCategories = (categoriesData?.data as CategoriesData)?.data || [];
        ...
        <>
            ...
            >
                <div className="px-4 sm:px-6 md:px-16 py-2 flex justify-between items-center">
                    ...
                    <span className="font-semibold uppercase">Free Shipping on Orders Over ৳ 5000</span>
            ...
            <div className="sticky top-0 z-40 bg-white border-b shadow-sm backdrop-blur-sm bg-white/95">
                <div className="px-4 sm:px-6 md:px-16">
                    <div className="flex items-center justify-between py-2">
                        <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} className="xl:hidden w-32 items-center justify-center flex">
                            <Button
                                variant="ghost"
                                size="icon"
                        ...
                        <Link href="/">
                            <Image
                                ...
                                priority
                            />
                            <div className="text-2xl font-bold text-orange-500 sr-only">YoungHeart</div>
                        </Link>
                    ...
                    <AnimatePresence>
                        {isSearchOpen && (
                            ...
                            >
                                <X className="absolute right-3 top-1 text-gray-400 cursor-pointer" onClick={() => setIsSearchOpen(false)} />
                                        ...
                                        if (category?.children && category?.children?.length > 0) {
                                            return (
                                                <MobileDropdown
                                                    ...
                                                    onToggle={() => toggleDropdown(`${category.id}-mobile`)}
                                                    onLinkClick={() => {
